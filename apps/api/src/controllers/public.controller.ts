...
export class PublicController {
  ...
  async addGallery(@Body() body: unknown) {
    ...
    const item = await this.prisma.galleryItem.create({
      data: { image: input.image, imageHash: createHash("sha256").update(input.image).digest("hex"), caption: input.caption, link: input.link ?? null, kind: input.kind ?? "image", title: input.title ?? null, category: input.category?.trim() ? input.category.trim() : null, thumb: input.thumb || null },
    });
