...
import { VideoProfile } from "./components/public/VideoProfile";
...
function PublicSite({ content }: { content: SiteContent }) {
  ...
  return <div className="min-h-full bg-cream font-sans text-midnight"><Header brand={content.brand} navigation={content.navigation} content={content} onAdmin={() => { window.location.hash = "admin"; }} /><main><Hero hero={content.hero} onDaftar={onDaftar} /><VideoProfile video={content.video} /><Marquee items={content.marquee} /><Stats items={content.stats} /><About about={content.about} /><DetailSections content={content} /><Programs programs={content.programs} onDaftar={onDaftar} /><Research research={content.research} /><Community community={content.community} /><StudentLife studentLife={content.studentLife} onDaftar={onDaftar} /><News news={content.news} /><CTA cta={content.cta} onDaftar={onDaftar} /></main><Footer footer={content.footer} onDaftar={onDaftar} /><BackToTop />{pmb && <Pmb onClose={() => setPmb(false)} programs={content.programs.cards.map((card) => card.title)} />}</div>;
}
