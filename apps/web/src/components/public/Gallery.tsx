...
function Viewer({ item }: { item: GalleryItem }) {
  ...
  return <DriveImage src={item.image} alt={item.caption || item.title || ""} className="max-h-[75vh] w-full rounded-2xl object-contain bg-black" />;
}
    ...
    <div className="mx-auto w-full max-w-4xl py-8" onClick={(e) => e.stopPropagation()}>
      ...
      {mode === "grid" && shown.length > 0 && <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
        {shown.map((item, i) => <button key={item.id} onClick={() => { setIndex(i); setMode("view"); }} className="group relative aspect-square overflow-hidden rounded-xl bg-white/10" aria-label={item.title || item.caption || `Item ${i + 1}`}>
          {item.kind === "video" ? <VideoThumb item={item} /> : <DriveImage src={item.image} alt="" className="h-full w-full object-cover transition group-hover:scale-105" />}
